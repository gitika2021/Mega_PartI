...
def run_simulation_for_masks(shape_file, save_prefix, #inrat,
                             num_simulations=1, total_masks=0, ldcr_grid_path='',N=1, org_lc_path="",nproc=32, random_seed = None):
    ...
    # param_sets = []
    # ids = rng.choice(ldcr_grid.shape[0], size=total_masks, replace=False)
    ...
    #         ids = rng.choice(ldcr_grid.shape[0], size=60000, replace=False) # this stops repetition
    #         #ids = 734
    #         a = ldcr_grid[ids,0][0]
    #         b = ldcr_grid[ids,1][0]
    ...
    total_lc = total_masks 
    ldcr_grid = np.load(ldcr_grid_path)
    #print('org_lc_path',org_lc_path)
    print(f"Generating parameter sets for {total_masks} masks sims...")
    param_sets = []
    
    indices = rng.choice(ldcr_grid.shape[0], size=total_masks, replace=False)# this stops repetition of indices
    for ids in indices:
        mask_params = []
        a = ldcr_grid[ids,0]
        b = ldcr_grid[ids,1]
        ratio = ldcr_grid[ids,3]
